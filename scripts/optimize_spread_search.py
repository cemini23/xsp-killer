#!/usr/bin/env python3
"""Stage B spread × window edge search under Nagus locks.

Sweeps debit_spread width_strikes {1,2,3} × windows {am, close} × volume
{vall, vq33}, plus one naked close control. Does **not** flip LIVE_ENTRIES /
LIVE_EXITS. Emitted YAML is always ``active: false``. Pricing is always
``modeled_bs_lite`` (never historical XSP chain).

Offline:  ``python scripts/optimize_spread_search.py --mode fixture -v``
Strict:   ``python scripts/optimize_spread_search.py --mode uw --period 5y \\
...
logger = logging.getLogger("xsp_killer.optimize_spread_search")
...
# Search windows only (am least-bad yesterday; close = structure baseline).
ENTRY_WINDOWS: dict[str, tuple[time, time]] = {
    "am": (time(9, 45), time(11, 0)),
    "close": (time(15, 45), time(16, 0)),
}

# Nagus locks for this search (CLI may override widths / windows / volume).
NAGUS_DTE = 30
...
def _vid(
    *,
    ...
    if structure_mode == "naked":
        return (
            f"ss_naked_{window_id}_dte{dte}_tp{tp_i}_sl{sl_i}_"
            f"{vol_label}_h{hold}"
        )
    return (
        f"ss_dspread_w{width_strikes}_{window_id}_dte{dte}_tp{tp_i}_sl{sl_i}_"
        f"{vol_label}_h{hold}"
    )
...
def _cell_patch(
    *,
    ...
    start, end = ENTRY_WINDOWS[window_id]
    entry: dict[str, Any] = {
        "dte_pick": "target",
        ...
        "debit_spread_width_strikes": int(width_strikes),
    }
    if vol is not None:
    ...
    return {
        ...
        "exit": {
            ...
            "swing_hold": False,
            "max_hold_dte": 0,
            # Preserve base friday_flatten from lane_a_rules via merge; pin
            # explicitly so research cells cannot silently drop the rule.
...
def build_spread_search_grid(
    *,
    widths: list[int] | None = None,
    windows: list[str] | None = None,
    volume_gates: list[float | None] | None = None,
    holds: list[int] | None = None,
    include_naked_control: bool = True,
    max_grid: int = MAX_GRID_DEFAULT,
    allow_large: bool = False,
) -> list[dict[str, Any]]:
    """Bounded cells: width × window × vol (+ optional naked close control)."""
    width_list = list(widths) if widths is not None else list(DEFAULT_WIDTHS)
    win_ids = list(windows) if windows is not None else list(DEFAULT_WINDOWS)
    for wid in win_ids:
    ...
    for w in width_list:
        if int(w) < 1:
            raise ValueError(f"width_strikes must be >= 1, got {w}")
    ...
    n_spread = len(width_list) * len(win_ids) * len(vols) * len(hold_list)
    n_control = len(hold_list) if include_naked_control else 0
    # Naked control is fixed to close + vall (ungated) — one cell per hold.
    n = n_spread + n_control
    if n > int(max_grid) and not allow_large:
        raise GridBudgetError(
            f"spread-search grid size {n} exceeds budget {max_grid}; "
            "narrow --widths / --windows / --volume-pctile or pass --allow-large"
        )

    cells: list[dict[str, Any]] = []
    for width, wid, vol, hold in product(width_list, win_ids, vols, hold_list):
        vlab = _vol_label(vol)
        vid = _vid(
        ...
        desc = (
            f"spread-search debit_spread w={width} window={wid} "
            f"[{_fmt_hhmm(start)},{_fmt_hhmm(end)}) dte={NAGUS_DTE} "
            f"tp={COARSE_TP} sl={COARSE_SL} early_sl={COARSE_SL_EARLY} "
            f"volume={vol_word} hold={hold}"
        )
        ...
        cells.append(
            {
                ...
                "is_naked_control": False,
                "spec": spec,
                "overrides": deepcopy(merged),
            }
        )

    if include_naked_control:
        for hold in hold_list:
            wid = "close"
            vol = None  # vall baseline
            vlab = _vol_label(vol)
            vid = _vid(
            ...
            patch = _cell_patch(
                ...
                window_id=wid,
                vol=vol,
                width_strikes=2,  # unused for naked; keep knob default
            )
            merged = _deep_merge(deepcopy(BASE_28DTE_ATM_OVERRIDES), patch)
            logging_cfg = merged.setdefault("logging", {})
            logging_cfg["logic_version"] = f"xsp_lane_a_{vid}"
            start, end = ENTRY_WINDOWS[wid]
            desc = (
                f"spread-search naked control window={wid} "
                f"[{_fmt_hhmm(start)},{_fmt_hhmm(end)}) dte={NAGUS_DTE} "
                f"tp={COARSE_TP} sl={COARSE_SL} volume=off hold={hold}"
            )
            spec = _spec_from_overrides(vid, merged, description=desc)
            spec = VariantSpec(
                variant_id=spec.variant_id,
            ...
            cells.append(
                {
                    "variant_id": vid,
                    "structure_mode": "naked",
                    "debit_spread_width_strikes": 0,
                    "window_id": wid,
                    "window_start_et": _fmt_hhmm(start),
                    "window_end_et": _fmt_hhmm(end),
                    "volume_gate_max_pctile": vol,
                    "max_hold_sessions": int(hold),
                    "is_naked_control": True,
                    "spec": spec,
                    "overrides": deepcopy(merged),
                }
            )
...
def _parse_widths(raw: str | None) -> list[int]:
    if raw is None or not str(raw).strip():
    ...
    for part in str(raw).split(","):
        ...
        if tok:
            out.append(int(tok))
    return out or list(DEFAULT_WIDTHS)
...
def _summarize_intraday(res: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "n_blocked_friday": int(getattr(res, "n_blocked_friday", 0) or 0),
...
def _report_to_markdown(payload: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# Spread × Window Search Report")
    lines.append("")
    ...
    lines.append(
        ...
        "Friday flatten + friday_no_entry apply in Stage B."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "| variant | structure | w | window | vol | n | mean% | win% | "
        "early_green | blocked | fri_block | friday_flatten | status |"
    )
    lines.append(
        "|---|---|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---|"
    )
    for r in ranking:
        ...
        fri_x = int(exits.get("friday_flatten") or 0)
        lines.append(
            f"| `{r.get('variant_id')}` | {r.get('structure_mode')} | "
            f"{r.get('debit_spread_width_strikes')} | {r.get('window_id')} | "
            f"{vol_text} | {r.get('n_trades')} | "
            f"{100 * float(r.get('mean_net_pnl_pct') or 0):.2f} | "
            f"{r.get('win_pct')} | "
            f"{float(r.get('early_green_rate') or 0):.2f} | "
            f"{r.get('n_entries_blocked', 0)} | "
            f"{r.get('n_blocked_friday', 0)} | {fri_x} | "
    ...
    if by_window:
        ...
        lines.append("")
        lines.append("| window | n_cells | mean_of_means% | total_n |")
        lines.append("|---|---:|---:|---:|")
        for wid, row in by_window.items():
            lines.append(
                f"| {wid} | {row.get('n_cells')} | "
                f"{100 * float(row.get('mean_of_means') or 0):.2f} | "
                f"{row.get('total_trades')} |"
            )
        lines.append("")

    by_width = payload.get("by_width") or {}
    if by_width:
        lines.append("## By width_strikes (debit_spread only)")
        lines.append("")
        lines.append("| width | n_cells | mean_of_means% | total_n |")
        lines.append("|---:|---:|---:|---:|")
        for w, row in sorted(by_width.items(), key=lambda kv: int(kv[0])):
            lines.append(
                f"| {w} | {row.get('n_cells')} | "
                f"{100 * float(row.get('mean_of_means') or 0):.2f} | "
                f"{row.get('total_trades')} |"
    ...
    lines.append(f"- window_id: `{rec.get('window_id')}`")
    lines.append(f"- width_strikes: `{rec.get('debit_spread_width_strikes')}`")
...
def write_spread_search_report(
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Stage B spread × window search (width × am/close × vol) under "
            "Nagus defaults + naked close control. Does not touch LIVE_*."
        )
    )
    p.add_argument(
        "--mode",
    ...
    p.add_argument(
        "--widths",
        default="1,2,3",
        help="Comma debit_spread width_strikes (default 1,2,3)",
    )
    p.add_argument(
        "--windows",
        default="am,close",
        help="Comma entry window ids (default am,close)",
    )
    p.add_argument(
        "--volume-pctile",
        default="0.33,none",
        help="Comma volume max pctiles; 'none' = ungated (default 0.33,none)",
    )
    p.add_argument(
        "--holds",
        default="5",
    ...
    p.add_argument(
        "--no-naked-control",
        action="store_true",
        help="Omit the naked close / vall control cell",
    )
    p.add_argument(
        "--split-frac",
        type=float,
    ...
    p.add_argument(
        ...
        help="Output directory for spread_search_*.json + .md",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        cells = build_spread_search_grid(
            ...
            holds=holds,
            include_naked_control=not bool(args.no_naked_control),
    ...
    with tempfile.TemporaryDirectory(prefix="xsp_ss_rules_") as tmp:
        tmp_path = Path(tmp)
        for cell in cells:
            ...
            summary["is_naked_control"] = bool(cell["is_naked_control"])
            summary["decision_status"] = "RESEARCH ONLY"
            summary["pricing_fidelity"] = "modeled_bs_lite"
            observations: list[tuple[str, float]] = []
            for t in res.trades:
                try:
                    sess = str(datetime.fromisoformat(t.entry_ts).date())
            ...
            logger.info(
                "%s structure=%s w=%s win=%s n=%d mean=%.4f "
                "blocked=%d fri_block=%d fri_flat=%d",
                spec.variant_id,
                cell["structure_mode"],
                cell["debit_spread_width_strikes"],
                cell["window_id"],
                summary["n_trades"],
                summary["mean_net_pnl_pct"],
                summary["n_entries_blocked"],
                summary["n_blocked_friday"],
                int((summary.get("exit_reasons") or {}).get("friday_flatten") or 0),
            )
    ...
    for r in ranking:
        ...
        bucket = by_window.setdefault(
            wid, {"n_cells": 0, "means": [], "total_trades": 0}
        )
        bucket["n_cells"] += 1
        bucket["means"].append(float(r.get("mean_net_pnl_pct") or 0.0))
        bucket["total_trades"] += int(r.get("n_trades") or 0)
        if r.get("structure_mode") == "debit_spread":
            wkey = str(int(r.get("debit_spread_width_strikes") or 0))
            wb = by_width.setdefault(
                wkey, {"n_cells": 0, "means": [], "total_trades": 0}
            )
            wb["n_cells"] += 1
            wb["means"].append(float(r.get("mean_net_pnl_pct") or 0.0))
            wb["total_trades"] += int(r.get("n_trades") or 0)
    ...
    payload["by_width"] = by_width

    # Prefer best debit_spread cell for recommendation; fall back to top row.
    rec_row = None
    for r in ranking:
        if r.get("structure_mode") == "debit_spread":
            rec_row = r
    ...
    edge_ok = False
    edge_reason = "spread_search_research_only"
    ...
    if rec_row is not None:
        ...
        entry_ov["structure_mode"] = rec_row.get("structure_mode", "debit_spread")
        entry_ov["debit_spread_width_strikes"] = int(
            rec_row.get("debit_spread_width_strikes") or 2
        )
        entry_ov["window_start_et"] = rec_row.get("window_start_et")
        entry_ov["window_end_et"] = rec_row.get("window_end_et")
    ...
    payload["recommendation"] = {
        ...
        "window_id": rec_row.get("window_id") if rec_row else None,
        "debit_spread_width_strikes": (
            rec_row.get("debit_spread_width_strikes") if rec_row else None
        ),
        "edge_ok": edge_ok,
    ...
    json_path, md_path = write_spread_search_report(payload, Path(args.out))
