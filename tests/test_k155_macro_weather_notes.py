...
def test_build_monitor_macro_weather_extras_k174_from_prod_config():
    ...
    assert extras["cf_weekend_depth"]["flag"] == "jul16_19_regime_packet"
    assert extras["cf_weekend_depth"]["view_changed_unwind_carry_weekend_depth"] is True
    # k251 overwrites lane_a_overnight with keep_tight_vs_k245
    assert extras["lane_a_overnight"]["keep_tight_vs_k245"] is True
...
def test_load_k231_notes_prod_config():
    notes = load_k231_notes()
    assert notes.get("version") == "2026-08-11"
    assert notes.get("needs_verification") is True
    fed = notes["fed_warsh_week"]
    assert fed["flag"] == "fed_decision_week_hyperscaler_capex_regime"
    assert fed["hyperscaler_capex_swing_for_index_vol"] is True
    assert fed["needs_verification"] is True
    meta = notes["meta_spot_watch_only"]
    assert meta["no_options_lotto"] is True
    assert meta["spot_watch_only"] is True
    rea = notes["rea_rare_earth_sentiment_only"]
    assert rea["not_xsp_tradable"] is True
    assert rea["sentiment_regime_only"] is True
    assert notes["lane_a_overnight"]["keep_tight_vs_k229"] is True
    assert notes["constraints"]["no_auto_watches_json_writes"] is True
    assert notes["constraints"]["playbook_snapshot_gates_unchanged"] is True


def test_build_monitor_macro_weather_extras_includes_k231(tmp_path: Path):
    cfg = tmp_path / "k155.yaml"
    cfg.write_text(
        yaml.safe_dump(
            {
                ...
                "k231": {
                    ...
                    "fed_warsh_week": {
                        "flag": "fed_decision_week_hyperscaler_capex_regime",
                        "hyperscaler_capex_swing_for_index_vol": True,
                        "needs_verification": True,
                    },
                    "hot_money_rotation": {
                        "flag": "ai_semis_flush_to_industrials_regime",
                        "ai_semis_hot_money_flushed": True,
                    },
                    "meta_spot_watch_only": {
                        "flag": "meta_earnings_hyperscaler_capex_tell",
                        "no_options_lotto": True,
                        "spot_watch_only": True,
                    },
                    "rea_rare_earth_sentiment_only": {
                        "flag": "western_re_rerate_watch_not_xsp_tradable",
                        "not_xsp_tradable": True,
                        "sentiment_regime_only": True,
                    },
                    "lane_a_overnight": {
                        "keep_tight_vs_k229": True,
                        "no_posture_change_on_teaser": True,
                    },
                    "constraints": {
                        "no_live_auto_calibrator": True,
    ...
    extras = build_monitor_macro_weather_extras(
        load_k155_notes(cfg),
        usdjpy=162.40,
        k231_notes=load_k231_notes(cfg),
        notes_path=cfg,
    )
    assert extras["k231_version"] == "2026-08-11"
    assert extras["meta_spot_watch_only"]["no_options_lotto"] is True
    assert extras["lane_a_overnight"]["keep_tight_vs_k229"] is True
    assert extras["constraints"]["playbook_snapshot_gates_unchanged"] is True


def test_build_monitor_macro_weather_extras_k231_from_prod_config():
    extras = build_monitor_macro_weather_extras(usdjpy=162.35)
    assert extras["k231_version"] == "2026-08-11"
...
def test_load_k232_notes_prod_config():
    notes = load_k232_notes()
    vol = notes["wublock_july_spot_volume"]
    assert notes.get("version") == "2026-08-11"
    assert notes.get("needs_verification") is True
    assert vol["flag"] == "crypto_spot_volume_contraction_regime_snapshot"
    assert vol["july_2026_spot_volume_429b"] is True
    assert vol["mom_decline_21_7pct_context"] is True
    assert vol["all_14_exchanges_down"] is True
    assert vol["wash_trading_bot_caveat"] is True
    assert vol["single_regime_exploratory_only"] is True
    assert vol["no_cross_regime_strategy_implication"] is True
    rel = notes["relative_strength_weakness"]
    assert rel["defi_milder_than_cex_aggregate_this_snapshot"] is True
    assert notes["lane_a_overnight"]["keep_tight_vs_k231"] is True
    assert notes["constraints"]["no_order_bots"] is True
    assert notes["constraints"]["no_auto_watches_json_writes"] is True
    assert notes["constraints"]["playbook_snapshot_gates_unchanged"] is True


def test_build_monitor_macro_weather_extras_includes_k232(tmp_path: Path):
    cfg = tmp_path / "k155.yaml"
    cfg.write_text(
        yaml.safe_dump(
            {
                ...
                "k232": {
                    ...
                    "wublock_july_spot_volume": {
                        "flag": "crypto_spot_volume_contraction_regime_snapshot",
                        "july_2026_spot_volume_429b": True,
                        "all_14_exchanges_down": True,
                        "single_regime_exploratory_only": True,
                        "needs_verification": True,
                    },
                    "relative_strength_weakness": {
                        "flag": "defi_milder_cex_sharper_mom_declines",
                        "defi_milder_than_cex_aggregate_this_snapshot": True,
                    },
                    "lane_a_overnight": {
                        "keep_tight_vs_k231": True,
                        "no_posture_change_on_teaser": True,
                    },
                    "constraints": {
                        "no_live_auto_calibrator": True,
    ...
    extras = build_monitor_macro_weather_extras(
        load_k155_notes(cfg),
        usdjpy=162.40,
        k232_notes=load_k232_notes(cfg),
        notes_path=cfg,
    )
    assert extras["k232_version"] == "2026-08-11"
    assert extras["wublock_july_spot_volume"]["july_2026_spot_volume_429b"] is True
    assert extras["lane_a_overnight"]["keep_tight_vs_k231"] is True
    assert extras["constraints"]["no_order_bots"] is True


def test_build_monitor_macro_weather_extras_k232_from_prod_config():
    extras = build_monitor_macro_weather_extras(usdjpy=162.35)
    vol = extras["wublock_july_spot_volume"]
    assert extras["k232_version"] == "2026-08-11"
    assert vol["flag"] == "crypto_spot_volume_contraction_regime_snapshot"
    assert vol["mom_decline_21_7pct_context"] is True
    ...
    assert extras["constraints"]["playbook_snapshot_gates_unchanged"] is True
    # k232 content flag retained even when constraints last-win from k233
    assert extras["wublock_july_spot_volume"]["needs_verification"] is True




def test_load_k233_notes_prod_config():
    notes = load_k233_notes()
    fed = notes["fed_speaks_fomc_iv_surface_arxiv_2608_10693"]
    wu = notes["wublock_july_derivatives_volume"]
    assert notes.get("version") == "2026-08-12"
    assert fed["flag"] == "prefomc_short_dated_otm_iv_timing_awareness"
    assert fed["iv_builds_before_scheduled_fomc"] is True
    assert fed["concentrated_short_dated_otm"] is True
    assert fed["amplified_in_high_vol_regimes"] is True
    assert fed["no_lane_retune_on_paper_forecasts"] is True
    assert fed["timing_regime_input_only"] is True
    assert wu["flag"] == "crypto_derivatives_volume_venue_vol_context"
    assert wu["hyperliquid_mom_decline_18_5pct_context"] is True
    assert wu["futures_spot_ratio_7_06x_up_13_6pct"] is True
    assert wu["no_auto_hl_perp_size_change"] is True
    assert wu["venue_vol_context_only"] is True
    assert notes["lane_a_overnight"]["keep_tight_vs_k232"] is True
    assert notes["constraints"]["no_lane_retune_on_paper_forecasts"] is True
    assert notes["constraints"]["no_auto_hl_perp_size_change"] is True
    assert notes["constraints"]["playbook_snapshot_gates_unchanged"] is True


def test_build_monitor_macro_weather_extras_includes_k233(tmp_path: Path):
    cfg = tmp_path / "k155.yaml"
    cfg.write_text(
        yaml.safe_dump(
            {
                ...
                "k233": {
                    ...
                    "fed_speaks_fomc_iv_surface_arxiv_2608_10693": {
                        "flag": "prefomc_short_dated_otm_iv_timing_awareness",
                        "no_lane_retune_on_paper_forecasts": True,
                        "timing_regime_input_only": True,
                    },
                    "wublock_july_derivatives_volume": {
                        "flag": "crypto_derivatives_volume_venue_vol_context",
                        "hyperliquid_mom_decline_18_5pct_context": True,
                        "no_auto_hl_perp_size_change": True,
                        "venue_vol_context_only": True,
                    },
                    "lane_a_overnight": {
                        "keep_tight_vs_k232": True,
                        "no_posture_change_on_teaser": True,
                    },
                    "constraints": {
                        "no_strategy_code": True,
    ...
    extras = build_monitor_macro_weather_extras(
        load_k155_notes(cfg),
        usdjpy=162.40,
        k233_notes=load_k233_notes(cfg),
        notes_path=cfg,
    )
    assert extras["k233_version"] == "2026-08-12"
    assert extras["fed_speaks_fomc_iv_surface_arxiv_2608_10693"][
        "no_lane_retune_on_paper_forecasts"
    ] is True
    assert extras["wublock_july_derivatives_volume"]["no_auto_hl_perp_size_change"] is True
    assert extras["lane_a_overnight"]["keep_tight_vs_k232"] is True


def test_build_monitor_macro_weather_extras_k233_from_prod_config():
    extras = build_monitor_macro_weather_extras(usdjpy=162.35)
    fed = extras["fed_speaks_fomc_iv_surface_arxiv_2608_10693"]
    wu = extras["wublock_july_derivatives_volume"]
    assert extras["k233_version"] == "2026-08-12"
    assert fed["iv_builds_before_scheduled_fomc"] is True
    assert fed["concentrated_short_dated_otm"] is True
    assert fed["amplified_in_high_vol_regimes"] is True
    assert fed["no_lane_retune_on_paper_forecasts"] is True
    assert wu["july_2026_derivatives_3_03t_context"] is True
    assert wu["hyperliquid_mom_decline_18_5pct_context"] is True
...
def test_load_k236_notes_prod_config():
    notes = load_k236_notes()
    traffic = notes["wublock_july_exchange_website_traffic"]
    assert notes.get("version") == "2026-08-14"
    assert traffic["flag"] == "crypto_exchange_website_traffic_venue_context"
    assert traffic["july_2026_visits_134_31m_context"] is True
    assert traffic["mom_decline_2_35pct_context"] is True
    assert traffic["twelve_exchanges_five_rose_seven_fell"] is True
    assert traffic["top3_binance_okx_coinbase_61_2pct_concentration"] is True
    assert traffic["deribit_website_mom_decline_44_7pct"] is True
    assert traffic["deribit_two_series_confirms_k233_options_venue_contraction"] is True
    assert traffic["top3_derivatives_share_74_9pct_vs_website_61_2pct_context"] is True
    assert traffic["geographic_binance_india_okx_japan_coinbase_usa_context"] is True
    assert traffic["mom_risers_bitfinex_kucoin_bybit_context_not_signal"] is True
    assert traffic["website_traffic_bot_marketing_confound"] is True
    assert traffic["venue_retail_flow_context_only"] is True
    assert traffic["no_strategy_code"] is True
    assert notes["lane_a_overnight"]["keep_tight_vs_k234"] is True
    assert notes["lane_a_overnight"]["no_posture_change_on_teaser"] is True
    assert notes["constraints"]["no_strategy_code"] is True
    assert notes["constraints"]["playbook_snapshot_gates_unchanged"] is True


def test_build_monitor_macro_weather_extras_includes_k236(tmp_path: Path):
    cfg = tmp_path / "k155.yaml"
    cfg.write_text(
        yaml.safe_dump(
            {
    ...
    extras = build_monitor_macro_weather_extras(
        load_k155_notes(cfg),
        usdjpy=162.40,
        k236_notes=load_k236_notes(cfg),
        notes_path=cfg,
    )
    assert extras["k236_version"] == "2026-08-14"
    assert (
        extras["wublock_july_exchange_website_traffic"]["flag"]
        == "crypto_exchange_website_traffic_venue_context"
    )
    assert extras["wublock_july_exchange_website_traffic"][
        "deribit_two_series_confirms_k233_options_venue_contraction"
    ] is True
    assert extras["wublock_july_exchange_website_traffic"][
        "website_traffic_bot_marketing_confound"
    ] is True
    assert extras["lane_a_overnight"]["keep_tight_vs_k234"] is True
    assert extras["constraints"]["playbook_snapshot_gates_unchanged"] is True


def test_build_monitor_macro_weather_extras_k236_from_prod_config():
    extras = build_monitor_macro_weather_extras(usdjpy=162.35)
    traffic = extras["wublock_july_exchange_website_traffic"]
    assert extras["k236_version"] == "2026-08-14"
    assert traffic["flag"] == "crypto_exchange_website_traffic_venue_context"
    assert traffic["july_2026_visits_134_31m_context"] is True
    assert traffic["mom_decline_2_35pct_context"] is True
...
def test_load_k240_notes_prod_config():
    notes = load_k240_notes()
    talon = notes["glitchspx_talon_hitl"]
    rssb = notes["moontower_rssb_corr_awareness"]
    hitl = notes["hitl_watch_policy"]
    assert notes.get("version") == "2026-08-17"
    assert talon["flag"] == "talon_weekly_sector_map_hitl_awareness"
    assert talon["weeklies_aug10_14_and_aug17_21_context"] is True
    assert talon["broad_bullish_conditional"] is True
    assert talon["week2_avg_snapshot_move_0_57pct_context"] is True
    assert talon["qqq_spxw_watchlist_not_chase"] is True
    assert talon["ote_break_hold_retest_framing"] is True
    assert talon["aggressiveness_from_qqq_spxw_sqqq_vxx_soxx_rut"] is True
    assert talon["theme_clusters_map_not_buy_list"] is True
    assert talon["no_lane_a_single_name_screen_rewrite"] is True
    assert talon["prototyping_ai_analyst_skylit_caveat"] is True
    assert talon["operator_hypothesis_required_before_lane_change"] is True
    assert talon["hitl_only"] is True
    assert talon["no_auto_arm"] is True
    assert rssb["flag"] == "rssb_implied_stock_bond_corr_readout_awareness"
    assert rssb["back_out_implied_corr_from_rssb_iv"] is True
    assert rssb["risk_parity_structurally_short_that_corr"] is True
    assert rssb["realized_vti_ief_30d_corr_0_43_context"] is True
    assert rssb["fisher_z_ci_wide_at_n22"] is True
    assert rssb["ci_95_approx_0_02_to_0_72_context"] is True
    assert rssb["do_not_treat_point_estimate_as_watch_level"] is True
    assert rssb["prefer_1y_ci_over_30d_point"] is True
    assert rssb["awareness_only"] is True
    assert hitl["flag"] == "no_auto_arm_watches_json"
    assert hitl["no_auto_arm_watches_json"] is True
    assert hitl["vivo_not_watch_unless_operator_names_symbol_op_level"] is True
    assert hitl["phase1_operator_read_only"] is True
    assert hitl["optional_hitl_watch_only_if_named_level_later"] is True
    assert notes["lane_a_overnight"]["keep_tight_vs_k236"] is True
    assert notes["lane_a_overnight"]["no_posture_change_on_teaser"] is True
    assert notes["constraints"]["no_auto_arm"] is True
    assert notes["constraints"]["no_auto_watches_json_writes"] is True
    assert notes["constraints"]["no_lane_a_screen_rewrite_from_talon"] is True
    assert notes["constraints"]["hitl_only"] is True
    assert notes["constraints"]["playbook_snapshot_gates_unchanged"] is True


def test_build_monitor_macro_weather_extras_includes_k240(tmp_path: Path):
    cfg = tmp_path / "k155.yaml"
    cfg.write_text(
        yaml.safe_dump(
            {
    ...
    extras = build_monitor_macro_weather_extras(
        load_k155_notes(cfg),
        usdjpy=162.40,
        k240_notes=load_k240_notes(cfg),
        notes_path=cfg,
    )
    assert extras["k240_version"] == "2026-08-17"
    assert extras["glitchspx_talon_hitl"]["flag"] == "talon_weekly_sector_map_hitl_awareness"
    assert extras["glitchspx_talon_hitl"]["theme_clusters_map_not_buy_list"] is True
    assert extras["moontower_rssb_corr_awareness"][
        "do_not_treat_point_estimate_as_watch_level"
    ] is True
    assert extras["hitl_watch_policy"]["no_auto_arm_watches_json"] is True
    assert extras["lane_a_overnight"]["keep_tight_vs_k236"] is True
    assert extras["constraints"]["hitl_only"] is True
    assert extras["constraints"]["playbook_snapshot_gates_unchanged"] is True


def test_build_monitor_macro_weather_extras_k240_from_prod_config():
    extras = build_monitor_macro_weather_extras(usdjpy=162.35)
    talon = extras["glitchspx_talon_hitl"]
    rssb = extras["moontower_rssb_corr_awareness"]
    hitl = extras["hitl_watch_policy"]
    assert extras["k240_version"] == "2026-08-17"
    assert talon["flag"] == "talon_weekly_sector_map_hitl_awareness"
    assert talon["weeklies_aug10_14_and_aug17_21_context"] is True
    assert talon["broad_bullish_conditional"] is True
    ...
    assert hitl["flag"] == "no_auto_arm_watches_json"
    assert hitl["no_auto_arm_watches_json"] is True
    assert hitl["operator_must_name_symbol_op_level"] is True
    assert extras["lane_a_overnight"]["keep_tight_vs_k245"] is True
    assert extras["lane_a_overnight"]["no_posture_change_on_teaser"] is True
    assert extras["constraints"]["no_auto_arm"] is True
    assert extras["constraints"]["no_auto_watches_json_writes"] is True
    assert extras["constraints"]["hitl_only"] is True
    assert extras["constraints"]["playbook_snapshot_gates_unchanged"] is True



def test_load_k245_notes_prod_config():
    notes = load_k245_notes()
    talon = notes["glitchspx_talon_aug24_hitl"]
    dash = notes["talon_dashboard_aug24"]
    hitl = notes["hitl_watch_policy"]
    assert notes.get("version") == "2026-08-22"
    assert talon["flag"] == "talon_weekly_aug24_28_hitl_map"
    assert talon["week_aug24_28_sheet_aug21_context"] is True
    assert talon["breadth_137_of_5_context"] is True
    assert talon["score_weight_96_2pct_context"] is True
    assert talon["avg_snapshot_plus_0_95pct_context"] is True
    assert talon["not_one_way_index_vol_map"] is True
    assert talon["size_from_dashboard_not_setup_count"] is True
    assert talon["theme_clusters_map_not_buy_list"] is True
    assert talon["no_lane_a_single_name_screen_rewrite"] is True
    assert talon["ote_break_hold_retest_framing"] is True
    assert talon["no_trade_if_extended_between_entry_and_first_target"] is True
    assert talon["prototyping_ai_analyst_skylit_caveat"] is True
    assert talon["operator_hypothesis_required_before_lane_change"] is True
    assert talon["hitl_only"] is True
    assert talon["no_auto_arm"] is True
    assert dash["flag"] == "dashboard_caps_size_not_breadth"
    assert dash["spxw_constructive_while_7573_50_holds_context"] is True
    assert dash["vxx_ote_18_50_context"] is True
    assert dash["vxx_warn_clears_19_50_while_iwm_or_smh_weaken"] is True
    assert dash["vix_healthier_fails_15_50_context"] is True
    assert dash["smh_ote_watch_context"] is True
    assert dash["iwm_rally_fail_near_300_context"] is True
    assert dash["awareness_only"] is True
    assert dash["not_a_watch_level"] is True
    assert hitl["flag"] == "no_auto_arm_watches_json"
    assert hitl["no_auto_arm_watches_json"] is True
    assert hitl["operator_must_name_symbol_op_level"] is True
    assert hitl["phase1_operator_read_only"] is True
    assert hitl["optional_hitl_watch_only_if_named_level_later"] is True
    assert notes["lane_a_overnight"]["keep_tight_vs_k240"] is True
    assert notes["lane_a_overnight"]["no_posture_change_on_teaser"] is True
    assert notes["constraints"]["no_auto_arm"] is True
    assert notes["constraints"]["no_auto_watches_json_writes"] is True
    assert notes["constraints"]["no_lane_a_screen_rewrite_from_talon"] is True
...
def test_build_monitor_macro_weather_extras_includes_k245(tmp_path: Path):
    cfg = tmp_path / "k155.yaml"
    cfg.write_text(
        yaml.safe_dump(
            {
    ...
    extras = build_monitor_macro_weather_extras(
        load_k155_notes(cfg),
        usdjpy=162.40,
        k245_notes=load_k245_notes(cfg),
        notes_path=cfg,
    )
    assert extras["k245_version"] == "2026-08-22"
    assert extras["glitchspx_talon_aug24_hitl"]["flag"] == "talon_weekly_aug24_28_hitl_map"
    assert extras["glitchspx_talon_aug24_hitl"]["theme_clusters_map_not_buy_list"] is True
    assert extras["talon_dashboard_aug24"]["not_a_watch_level"] is True
    assert extras["hitl_watch_policy"]["no_auto_arm_watches_json"] is True
    assert extras["lane_a_overnight"]["keep_tight_vs_k240"] is True
    assert extras["constraints"]["hitl_only"] is True
    assert extras["constraints"]["playbook_snapshot_gates_unchanged"] is True


def test_build_monitor_macro_weather_extras_k245_from_prod_config():
    extras = build_monitor_macro_weather_extras(usdjpy=162.35)
    talon = extras["glitchspx_talon_aug24_hitl"]
    dash = extras["talon_dashboard_aug24"]
    hitl = extras["hitl_watch_policy"]
    assert extras["k245_version"] == "2026-08-22"
    assert talon["flag"] == "talon_weekly_aug24_28_hitl_map"
    assert talon["week_aug24_28_sheet_aug21_context"] is True
...
def test_load_k251_notes_prod_config():
    notes = load_k251_notes()
    talon = notes["glitchspx_talon_aug31_hitl"]
    dash = notes["talon_dashboard_aug31"]
    hitl = notes["hitl_watch_policy"]
    assert notes.get("version") == "2026-08-30"
    assert talon["flag"] == "talon_weekly_aug31_sep4_hitl_map"
    assert talon["week_aug31_sep4_sheet_aug30_context"] is True
    assert talon["breadth_66_of_2_context"] is True
    assert talon["score_weight_97pct_context"] is True
    assert talon["avg_snapshot_minus_0_92pct_context"] is True
    assert talon["index_dashboard_defensive"] is True
    assert talon["spxw_ote_watch_not_chase"] is True
    assert talon["first_target_rr_0_1_to_1_context"] is True
    assert talon["spy_qqq_iwm_rally_fail_maps"] is True
    assert talon["size_from_dashboard_not_setup_count"] is True
    assert talon["theme_clusters_map_not_buy_list"] is True
    assert talon["no_lane_a_single_name_screen_rewrite"] is True
    assert talon["ote_break_hold_retest_framing"] is True
    ...
    assert talon["hitl_only"] is True
    assert talon["no_auto_arm"] is True
    assert dash["flag"] == "defensive_dashboard_caps_size"
    assert dash["spxw_ote_not_chase"] is True
    assert dash["spy_qqq_iwm_rally_fail_context"] is True
    assert dash["vxx_vol_warning_context"] is True
    assert dash["smh_constructive_only_around_lower_floor"] is True
    assert dash["awareness_only"] is True
    assert dash["not_a_watch_level"] is True
    assert hitl["flag"] == "no_auto_arm_watches_json"
    assert hitl["no_auto_arm_watches_json"] is True
    ...
    assert hitl["phase1_operator_read_only"] is True
    assert hitl["optional_hitl_watch_only_if_named_level_later"] is True
    assert notes["lane_a_overnight"]["keep_tight_vs_k245"] is True
    assert notes["lane_a_overnight"]["no_posture_change_on_teaser"] is True
    assert notes["constraints"]["no_auto_arm"] is True
    assert notes["constraints"]["no_auto_watches_json_writes"] is True
    assert notes["constraints"]["no_lane_a_screen_rewrite_from_talon"] is True
...
def test_build_monitor_macro_weather_extras_includes_k251(tmp_path: Path):
    cfg = tmp_path / "k155.yaml"
    cfg.write_text(
        yaml.safe_dump(
            {
    ...
    extras = build_monitor_macro_weather_extras(
        load_k155_notes(cfg),
        usdjpy=162.40,
        k251_notes=load_k251_notes(cfg),
        notes_path=cfg,
    )
    assert extras["k251_version"] == "2026-08-30"
    assert extras["glitchspx_talon_aug31_hitl"]["flag"] == "talon_weekly_aug31_sep4_hitl_map"
    assert extras["glitchspx_talon_aug31_hitl"]["theme_clusters_map_not_buy_list"] is True
    assert extras["talon_dashboard_aug31"]["not_a_watch_level"] is True
    assert extras["hitl_watch_policy"]["no_auto_arm_watches_json"] is True
    assert extras["lane_a_overnight"]["keep_tight_vs_k245"] is True
    assert extras["constraints"]["hitl_only"] is True
    assert extras["constraints"]["playbook_snapshot_gates_unchanged"] is True
...
def test_build_monitor_macro_weather_extras_k251_from_prod_config():
    extras = build_monitor_macro_weather_extras(usdjpy=162.35)
    talon = extras["glitchspx_talon_aug31_hitl"]
    dash = extras["talon_dashboard_aug31"]
    hitl = extras["hitl_watch_policy"]
    assert extras["k251_version"] == "2026-08-30"
    assert talon["flag"] == "talon_weekly_aug31_sep4_hitl_map"
    assert talon["week_aug31_sep4_sheet_aug30_context"] is True
...
def test_run_monitor_attaches_macro_weather_extras(tmp_path, monkeypatch):
    ...
    assert report.macro_weather_extras["k236_version"] == "2026-08-14"
    assert report.macro_weather_extras["k240_version"] == "2026-08-17"
    assert report.macro_weather_extras["k245_version"] == "2026-08-22"
    assert report.macro_weather_extras["k251_version"] == "2026-08-30"
    ...
    assert "fed_warsh_week" in report.macro_weather_extras
    assert "hot_money_rotation" in report.macro_weather_extras
    assert "meta_spot_watch_only" in report.macro_weather_extras
    assert "rea_rare_earth_sentiment_only" in report.macro_weather_extras
    assert "wublock_july_spot_volume" in report.macro_weather_extras
    ...
    assert "wublock_july_exchange_website_traffic" in report.macro_weather_extras
    assert "glitchspx_talon_hitl" in report.macro_weather_extras
    assert "moontower_rssb_corr_awareness" in report.macro_weather_extras
    assert "glitchspx_talon_aug24_hitl" in report.macro_weather_extras
    assert "talon_dashboard_aug24" in report.macro_weather_extras
    assert "glitchspx_talon_aug31_hitl" in report.macro_weather_extras
    assert "talon_dashboard_aug31" in report.macro_weather_extras
    assert "hitl_watch_policy" in report.macro_weather_extras
    assert "constraints" in report.macro_weather_extras
    assert "events" in report.macro_weather_extras
    assert report.macro_weather_extras["lane_a_overnight"]["keep_tight_vs_k245"] is True
    assert (
    ...
    assert report.macro_weather_extras["meta_spot_watch_only"]["no_options_lotto"] is True
    assert report.macro_weather_extras["rea_rare_earth_sentiment_only"]["not_xsp_tradable"] is True
    assert report.macro_weather_extras["wublock_july_spot_volume"]["single_regime_exploratory_only"] is True
    assert report.macro_weather_extras["constraints"]["no_auto_watches_json_writes"] is True
    assert report.macro_weather_extras["constraints"]["no_order_bots"] is True
    assert report.macro_weather_extras["wublock_july_spot_volume"]["needs_verification"] is True
    assert report.macro_weather_extras["constraints"]["playbook_snapshot_gates_unchanged"] is True
    assert (
        report.macro_weather_extras["fed_speaks_fomc_iv_surface_arxiv_2608_10693"][
            "no_lane_retune_on_paper_forecasts"
        ]
        is True
    )
    assert (
        report.macro_weather_extras["wublock_july_derivatives_volume"][
            "no_auto_hl_perp_size_change"
        ]
        is True
    )
    ...
    assert (
        report.macro_weather_extras["wublock_july_exchange_website_traffic"][
            "deribit_two_series_confirms_k233_options_venue_contraction"
        ]
        is True
    )
    ...
    assert (
        report.macro_weather_extras["glitchspx_talon_hitl"][
            "theme_clusters_map_not_buy_list"
        ]
        is True
    )
    assert (
        report.macro_weather_extras["moontower_rssb_corr_awareness"][
            "do_not_treat_point_estimate_as_watch_level"
        ]
        is True
    )
    assert (
        report.macro_weather_extras["hitl_watch_policy"]["no_auto_arm_watches_json"]
        is True
    )
    assert report.macro_weather_extras["constraints"]["no_auto_arm"] is True
    assert report.macro_weather_extras["constraints"]["no_auto_watches_json_writes"] is True
    assert report.macro_weather_extras["constraints"]["hitl_only"] is True
    assert report.macro_weather_extras["constraints"]["playbook_snapshot_gates_unchanged"] is True
    assert report.macro_weather_extras["talon_dashboard_aug24"]["not_a_watch_level"] is True
    assert report.macro_weather_extras["talon_dashboard_aug31"]["not_a_watch_level"] is True
    assert report.macro_weather_extras["hitl_watch_policy"]["operator_must_name_symbol_op_level"] is True
